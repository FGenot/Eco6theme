$(document).ready(function () {
	...
	function make_modal_add_resource(resource_name, img) {
		...
		modal_add_resource += '<div>';
		modal_add_resource += '<span>'+Malus(resource_name)+'</span>';
		modal_add_resource += '</div>';
		modal_add_resource += '</div>';
		// Ajout bonus element
		modal_add_resource += '<div class="row">';
		modal_add_resource += '<span class="bold" style="font-size: 16px;">Bonus liées à cette ressource :</span><br>';
		modal_add_resource += '<div>';
		modal_add_resource += '<span>'+Bonus(resource_name)+'</span>';
		modal_add_resource += '</div>';
		modal_add_resource += '</div>';
		// Ajout cout element
		modal_add_resource += '<div class="row">';
		modal_add_resource += '<span class="bold" style="font-size: 16px;">Cout liées à cette ressource :</span><br>';
		modal_add_resource += '<div>';
		modal_add_resource += '<span>'+MalusBadges(resource_name)+'</span>';
		modal_add_resource += '</div>';
		modal_add_resource += '</div>';
		modal_add_resource += '<div id="section_send" style="" class="section_send">';
		modal_add_resource += '<button style="margin-right: 5px;" class="btn red darken-2 delete_idea modal-close" onclick="validation("'+resource_name+'")" name="delete_idea" id="delete_idea">Annuler</button>';
		modal_add_resource += '<button class="btn green modal-close" name="sendIdea" onclick="validation('+"'"+resource_name+"'"+')" id="send_idea">Envoyer</button>';
		modal_add_resource += '</div>';
...
function validation(element){
	console.log(element);
	document.getElementById('selectButton').innerText = element;
	var chemin = document.getElementById('selectButton').innerText;
	
	if (chemin == "Arbre") {
		document.getElementById('imgSelect').setAttribute("src", "img/cherry-tree2.png");
	}
	if (chemin == "Imeuble") {
		document.getElementById('imgSelect').setAttribute("src", "img/building2.png");
	}
	if (chemin == "Route") {
		document.getElementById('imgSelect').setAttribute("src", "img/road2.png");
	}
	if (chemin == "Maison") {
		document.getElementById('imgSelect').setAttribute("src", "img/solar-house2.png");
	}
